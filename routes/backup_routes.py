...
def setup_backup_routes(memory_manager, preset_manager, skills_manager) -> APIRouter:
    ...
    async def import_data(request: Request):
        ...
        if "memories" in body and isinstance(body["memories"], list):
            import_entries = getattr(memory_manager, "import_entries", None)
            has_real_import_entries = callable(import_entries) and any(
                "import_entries" in vars(cls)
                for cls in type(memory_manager).__mro__
            )
            if has_real_import_entries:
                added = import_entries(body["memories"], owner=user)
            else:
